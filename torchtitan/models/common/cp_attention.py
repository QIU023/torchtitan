...
class CPInnerAttention:
    """Inner attention with backend-specific context-parallel behavior.

    Each backend implements its CP attention algorithm and tensor movement in
    ``forward``. Its config implements ``cp_shard_metadata`` for the metadata
    required by that attention layout. Model inputs and metadata use the same
    per-batch load balancer so their token partitions remain aligned.
    """
