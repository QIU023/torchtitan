...
class CPInnerAttention(ABC):
    """Inner attention that owns its context-parallel implementation.

    Each subclass owns two parts:

    1. ``forward`` implements the CP attention algorithm and tensor movement.
    2. ``cp_shard_metadata`` shards metadata for that attention layout.

    Model inputs are sharded before backend selection. Inputs and metadata use
    the same per-batch load balancer so their token partitions remain aligned.
    """
