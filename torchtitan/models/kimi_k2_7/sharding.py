...
def set_moonvit_sharding_config(
    ...
    *,
    ...
    ``include_cp_axis`` declares every state and activation replicated on the
    cp axis too, for a model whose decoder runs context parallel.
    """
    cp = {CP: spmd.R} if include_cp_axis else {}
    # The encoder's own ``pos_embed`` table is invariant across TP ranks.
    ve_cfg.sharding_config = ShardingConfig(
        state_shardings={
            "pos_embed": SpmdType({DP: spmd.R, **cp, TP: spmd.I}),
        },
        out_src_shardings=SpmdType({DP: spmd.V, **cp, TP: spmd.I}),
        out_dst_shardings=SpmdType({DP: spmd.V, **cp, TP: spmd.R}),
    )
    ve_cfg.rotary_pos_emb.sharding_config = ShardingConfig(
        state_shardings={
            "inv_freq": SpmdType({DP: spmd.R, **cp, TP: spmd.I}),
        },
        out_src_shardings=SpmdType({DP: spmd.R, **cp, TP: spmd.I}),
    )
