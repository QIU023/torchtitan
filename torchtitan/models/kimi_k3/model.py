    ...
    * K is split into ``kv_lora_rank + qk_rope_head_dim`` halves from
    ...
    def make_config(config: KimiK3Config, layer_idx: int) -> "KimiMLAAttention.Config":
        ...
        if config.q_lora_rank is None:
            ...
            cfg.q_a_proj = _lin(
                config.hidden_size, config.q_lora_rank, sharding=tp_replicate()
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = to_local_if_dtensor(attn_out)
        ...
        # to_local_if_dtensor round trip: the unwrap is a kernel-call detail and
    ...
    def _forward_cp(self, x: torch.Tensor, cp_group) -> torch.Tensor:
        ...
        q_BLE = to_local_if_dtensor(q_BLE)
        kv_BLF = to_local_if_dtensor(kv_BLF)
        ...
        k_rot_BLR = to_local_partial_grad(k_rot_BLR)
        ...
        attn_BTGV = to_local_if_dtensor(attn_BTGV)
        out_src_dim, out_dst_dim = ULYSSES.out_dims()
        attn_BLHV = cp_all_to_all_headseq(
        ...
        if self.mla_gated:
            ...
            # widths line up.
            attn_BLE = attn_BLE * to_local_if_dtensor(
...
class KimiK3TransformerBlock(Module, UpstreamFSDPNames):
    ...
    def make_config(
        ...
        def _norm() -> "RMSNorm.Config":
            # norm_config, not tp_replicate: the layer norms sit on the activation
