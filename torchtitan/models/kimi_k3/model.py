...
class KimiK3TransformerBlock(Module):
    ...
    def forward(
        ...
        if self.attention is not None:
            ...
            h_TD = self.delta_attention(h_TD, None, positions, cu_seqlens=cu_seqlens)
...
class KimiK3Model(Decoder):
    ...
    def encode_images(
        ...
        group_all = self._context_parallel_group()
        ...
        return self._encode_images_partitioned(pixel_values, grid_thw, group_all, subgroups)
    ...
    def _encode_images_partitioned(
        ...
        for p in range(n_passes):
            ...
            gathered = _PlainGradBoundary.apply(
                funcol.all_gather_tensor(local_feat.contiguous(), gather_dim=0, group=group)
            )
            if img is not None:
                t, h, w = grids[img]
