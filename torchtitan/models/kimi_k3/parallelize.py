...
def _kimi_k3_first_stage_modules(model: nn.Module) -> tuple[str, ...]:
    """Modules the split pins next to the embedding: the vision tower."""
    return (
        ("vision_encoder",)
...
def pipeline_kimi_k3(model: nn.Module, *, attn_res_cache: bool = True, **kwargs):
    ...
    """
    # Core's split places the embedding, the layers and the head; this model
    # pins the AttnRes aggregation modules to the stage that holds the head
    # (the final block attention runs there) and the vision tower to the stage
    # that holds the embedding (vision features are spliced into the
    # embeddings; nothing vision-side crosses a stage boundary).
