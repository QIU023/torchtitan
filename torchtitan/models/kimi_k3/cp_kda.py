...
# TODO(acisseJZhong): Expose the per-batch token partition from
# ContextParallelLoadBalancer so model-specific kernels can build routing from
# the exact partition used to shard inputs instead of reimplementing each load
# balancer's partition logic.
...
class ContextParallelInnerKDA(CPInnerAttention, InnerKDA):
    ...
    class Config(CPInnerAttention.Config, InnerKDA.Config):
        def cp_shard_metadata(
            ...
            # Kimi K3 validation permits only contiguous and head-tail partitions.
            load_balancer_type = (
                "headtail"
                if isinstance(load_balancer.config, HeadTailLoadBalancer.Config)
                else None
            )
