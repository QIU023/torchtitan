...
class TestWeightDecayScope(unittest.TestCase):
    ...
    def test_matrix_parameters_still_reach_muon(self):
        ...
        for name in muon_named:
            # Grouped-expert tensors carry shape-suffix names (w1_EFD); they
            # are 3-D, so inside the Muon group they take step()'s non-matrix
            # fallback -- same behavior the reference tree gave its experts.
            last = name.rsplit(".", 1)[1]
            self.assertTrue(
                last == "weight" or last[0] == "w", name
            )
...
class TestPerHeadMuonTagging(unittest.TestCase):
    ...
    def _model(self):
        ...
        # Structure is all the tagging reads, so build on meta: shapes exist,
        # no weights are allocated.
        with torch.device("meta"):
    ...
    def test_tags_qkv_on_both_attention_types(self):
        ...
        self.assertEqual(
            leaves, {"wq_b", "wkv_b", "q_proj", "k_proj", "v_proj"}
        )
