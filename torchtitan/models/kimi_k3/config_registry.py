...
def kimi_linear_debugmodel() -> Trainer.Config:
    """The Kimi-Linear-48B-A3B topology at debug extents, on text data.

    The released Kimi Linear is text-only, so this recipe reads the packed
    text dataset the other text debug models use rather than K3's multimodal
    loader; everything else follows ``kimi_k3_debugmodel``.
    """
    model_spec = model_registry("kimi_linear_debugmodel")
    return Trainer.Config(
        loss=ChunkedLossWrapper.Config(
            loss_fn=CrossEntropyLoss.Config(
                global_vocab_size=decoder_vocab_size(model_spec),
