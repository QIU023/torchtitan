...
@dataclass
class CPPatchPlan:
    """Vision attention for one image partitioned along the patch dimension (dynamic CP).

    Follows the dynamic vision context parallelism of the Kimi K3 report
    (section 5.2.3): a large image is split along its patch dimension across the
    ranks of a CP (sub)group, and attention gathers keys and values across those
    ranks.
    ...
    if they were rank 0's. The tables are built for the whole image and sliced.
    """
...
class KimiK3VisionCPAttention(VisionAttention):
    ...
    def forward(
        ...
        *,
        ...
        # The mask is index arithmetic on the keep vector, outside the SPMD
        # type checker as the other vision masks are.
        with spmd.no_typecheck():
...
class KimiK3VisionEncoder(MoonViTEncoder):
    ...
    def forward(
        ...
        *,
        ...
        """The shared tower's forward, plus the dynamic-CP patch partition.
        ...
        Kimi K3 feature and k2.5 has no use for it.
        """
        if cp_plan is None:
