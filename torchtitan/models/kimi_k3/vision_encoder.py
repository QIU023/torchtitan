...
class CPPatchPlan:
    ...
    This is the half that an image-level round-robin does NOT provide, and it is
    the load-bearing one -- the report's stated purpose for it is to reduce "the
    encoder latency of large visual samples and the cross-device load
    imbalance".
...
class KimiK3VisionEncoder(MoonViTEncoder):
    ...
    def _run_blocks(self, x, *, rope_cache, cp_plan):
        # No attention mask: a patch plan describes ONE image, so there are no
        # segment boundaries for a block-diagonal mask to express, and the
        # padded-key mask the partitioned attention needs is built from the
        # plan inside KimiK3VisionCPAttention.
