...
"""Context parallelism for the K3 attention layers: contracts, Ulysses, KCP plans.
...
declaring both here would be two mesh axes on tensor dim 2, which SpmdType
...
def _cp(axis_type: spmd.PerMeshAxisSpmdType) -> SpmdType:
    return SpmdType({CP: axis_type})
...
class CPContract:
    ...
    def redistributes(self) -> bool:
        ...
        return _per_axis_types(self.in_src) != _per_axis_types(self.in_dst)
...
def _shard_dim(layout: SpmdType) -> int:
    axis_type = _per_axis_types(layout)[CP]
...
def full_sequence_document_mask(attn, positions_L, cp_group):
    ...
    # Both mods, exactly as the non-CP path composes them
    # (Decoder._create_flex_attention_mask_for_document): the document mod
    # alone admits attention to later tokens of the same document, so causal
    # has to ride with it.
