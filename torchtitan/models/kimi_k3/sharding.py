...
KCP is declared as an identity pair rather than a placement change: the
recurrence passes state from rank to rank, which is a sequential dependency and
not a redistribution, so no placement pair describes it. ``KCP.redistributes()``
returning False is what says so, and the CP contract tests pin it.
"""
