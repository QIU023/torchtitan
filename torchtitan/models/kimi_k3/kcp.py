...
Two cross-rank dependencies with different shapes. The recurrence needs each
rank's true incoming state, which does NOT decompose by summation -- the delta
rule applies a token-dependent transition, so a prefix scan over (cumulative
transition, zero-started state) fragments recovers it. The short convolutions
need only the previous rank's tail, one fixed-size exchange.
"""
