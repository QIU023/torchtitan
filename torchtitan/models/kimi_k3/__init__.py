    ...
    *,
    ...
    return KimiLatentMoE.Config(
        ...
        routed_experts=RoutedExperts.Config(
            ...
            # dispatch when the ep mesh is None. "moonep" is Kimi K3's own
            # transport and stays in the model folder, like fla. Either way
            # the routed experts consume the LATENT stream, so the dispatcher
            # buffers size by latent_dim, not model dim.
            token_dispatcher=(
...
def _report_arch(attn_backend: str, moe_comm_backend: str) -> KimiK3Model.Config:
    ...
    return _kimi_k3_config(
        ...
        vocab_size=163840,
        num_layers=13,
        full_attention_layers=kimi_k3_full_attention_layers(13),
...
def _k3mini(attn_backend: str, moe_comm_backend: str) -> KimiK3Model.Config:
    ...
    return _kimi_k3_config(
        ...
        vocab_size=163840,
        num_layers=21,
        full_attention_layers=kimi_k3_full_attention_layers(21),
        attn_res_block_size=12,
        num_heads=4,
        q_lora_rank=128,
        kv_lora_rank=512,
