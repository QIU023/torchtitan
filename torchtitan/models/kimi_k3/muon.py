...
def tag_per_head_muon(model: nn.Module) -> int:
    ...
    for module in model.modules():
        if isinstance(module, KimiMLAAttention):
            names, heads = _PER_HEAD_MLA, module.n_heads
...
_MUON_EXCLUDE_DECAY_PATTERNS = (
    r".*embed_tokens.*",
    # The tower's learned positional table: embedding-family, not a matrix
    # parameter. The text-only reference never had it in scope.
    r".*pos_embed.*",
    r".*lm_head.*",
