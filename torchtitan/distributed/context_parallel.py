...
"""Context-parallel load-balancing APIs."""
...
from torchtitan.config.configs import LoadBalancer
...
class ContextParallelLoadBalancer(LoadBalancer):
    """Build and apply one batch's shared context-parallel partition.

    The base configuration uses contiguous sharding; subclasses provide a
    load-balanced partition for one batch.

    Built-in configurations are:

    - ``ContextParallelLoadBalancer.Config`` for contiguous CP sharding.
    - ``HeadTailLoadBalancer.Config`` for SDPA-based CP.
    - ``PTRRLoadBalancer.Config`` for FlexAttention-based CP.

    One instance is shared by input and attention-metadata sharding so both use
    the same per-batch partition.

    Args:
        config: Load-balancer configuration.
        input_dict: Model inputs used to derive the per-batch partition.
    ...
    class Config(LoadBalancer.Config):
        """Configuration for contiguous context-parallel sharding."""
    ...
    def shard_inputs(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        for name, buffer in zip(shard_names, sharded_buffers):
...
class HeadTailLoadBalancer(ContextParallelLoadBalancer):
    ...
    def __init__(
        ...
        *,
        ...
        input_T = cast(torch.Tensor, input_dict[input_name])
        seq_len = input_T.size(self._shard_dims[input_name])
