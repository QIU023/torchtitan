...
def clip_grad_norm_(
    ...
    # Under TP a model with undeclared (replicated) modules holds grads on two
    # meshes, (fsdp, tp) and (fsdp,); the norm is taken per mesh and combined.
    total_norm = _total_norm_by_mesh(grads, norm_type, error_if_nonfinite, foreach)
    ...
    _clip_grads_by_mesh(parameters, max_norm, total_norm, foreach)
    return total_norm
