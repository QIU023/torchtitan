...
def add_zero_valued_dependency(
    ...
    that only some data-parallel ranks execute -- a vision encoder on a batch
    that happens to carry no images, for example -- would therefore issue
    collectives on a subset of the process group and deadlock the step.
