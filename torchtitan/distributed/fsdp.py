...
def add_zero_valued_dependency(
    ...
    that only some data-parallel ranks execute -- a vision encoder whose output
    a rank consumes only partially under context parallel, for example -- would
    therefore issue collectives on a subset of the process group and deadlock
    the step.

    A rank with no real work for such a module runs it anyway and routes the
    result through this helper. Scaling by zero leaves ``output`` numerically
    unchanged while preserving the graph edge, so every rank issues the same
    collectives and the module receives zero gradients -- which is also its
