...
from torchtitan.experiments.kimi_k3.model import KimiLinearConfig, KimiMLAAttention
...
class TestMLAQLoRA(unittest.TestCase):
    def test_none_path_unchanged(self):
        attn = KimiMLAAttention(_cfg(None), layer_idx=0)
    ...
    def test_compression_pair_shapes(self):
        attn = KimiMLAAttention(_cfg(48), layer_idx=0)
    ...
    def test_both_paths_same_output_shape(self):
        torch.manual_seed(0)
        x = torch.randn(2, 6, 64)
        a = KimiMLAAttention(_cfg(None), layer_idx=0)(x)
        b = KimiMLAAttention(_cfg(48), layer_idx=0)(x)
    ...
    def test_official_k3_rank_builds(self):
        ...
        attn = KimiMLAAttention(_cfg(1536), layer_idx=0)
