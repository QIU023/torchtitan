...
class KimiMLAAttention(nn.Module):
    ...
    * K is split into ``kv_lora_rank + qk_rope_head_dim`` halves from
    ...
    def __init__(self, config: KimiLinearConfig, layer_idx: int) -> None:
        ...
        if self.mla_gated:
            self.attn_gate_proj = Linear(self.hidden_size, self.num_heads, bias=True)
    ...
    def _forward_cp(self, x: torch.Tensor, cp_group) -> torch.Tensor:
        ...
        if self.mla_gated:
            ...
            gate_BLH = torch.sigmoid(_to_local_if_dtensor(self.attn_gate_proj(x)))
            attn_BLHV = attn_BLHV * gate_BLH.unsqueeze(-1)
        out = self.o_proj(attn_BLHV.reshape(B, t_loc, h_loc * self.v_head_dim))
        return out
...
class KimiDeltaAttention(nn.Module):
    ...
    def _forward_cp(self, x: torch.Tensor, cp_group) -> torch.Tensor:
        ...
        g_out_BLHK = _local_linear(self.g_b_proj, _local_linear(self.g_a_proj, x)).view(
            B, t_loc, num_heads, head_dim
        )
