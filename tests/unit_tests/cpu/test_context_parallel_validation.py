...
from torchtitan.config import LoadBalancer, ParallelismConfig
...
class TestUlyssesConfigValidation(unittest.TestCase):
    ...
    def _config(
        *,
        cp: int = 2,
        tp: int = 1,
        load_balancer: LoadBalancer.Config | None = None,
    ...
    def test_rejects_the_default_headtail_load_balancer(self):
        ...
        with self.assertRaisesRegex(ValueError, "contiguous sharding"):
            config.__post_init__()

    def test_allows_explicit_contiguous_load_balancer(self):
        from torchtitan.distributed.context_parallel import ContextParallelLoadBalancer

        config = self._config(load_balancer=ContextParallelLoadBalancer.Config())
        config.__post_init__()
    ...
    def test_rejects_unknown_load_balancer(self):
        class OtherLoadBalancer(LoadBalancer):
            @dataclass(kw_only=True, slots=True)
