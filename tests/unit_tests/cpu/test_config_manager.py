...
class TestConfigManager(unittest.TestCase):
    ...
    def test_pipeline_split_is_given_one_way(self):
        """module_fqns_per_model_part and pipeline_parallel_layers_per_stage
        both describe the split; the config refuses both at once."""
        ParallelismConfig(module_fqns_per_model_part=[["tok_embeddings"]])
        ParallelismConfig(pipeline_parallel_layers_per_stage=2)
        with pytest.raises(ValueError, match="both describe the pipeline split"):
            ParallelismConfig(
