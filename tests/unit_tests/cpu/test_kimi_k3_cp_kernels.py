...
class TestKimiK3CPKernels(unittest.TestCase):
    def test_every_layer_gets_its_cp_kernel(self):
        config = kimi_k3_configs["debugmodel"][0]("flex", "standard")
        ...
        first = mla_layers[0].attention
        assert first is not None
        flex = first.inner_attention
        assert isinstance(flex, FlexAttention.Config)
        first.inner_attention = replace(flex, block_size=64)
        ...
        swapped = first.inner_attention
        assert isinstance(swapped, UlyssesCPFlexAttention.Config)
        self.assertEqual(swapped.block_size, 64)
        for layer in kda_layers:
            delta_attention = layer.delta_attention
            assert delta_attention is not None
            inner = delta_attention.inner_kda
    ...
    def test_ulysses_needs_flex_attention(self):
        config = kimi_k3_configs["debugmodel"][0]("flex", "standard")
        attention = next(l.attention for l in config.layers if l.attention is not None)
        assert attention is not None
        attention.inner_attention = VarlenAttention.Config()
    ...
    def test_cp_kernel_boundary_keeps_tokens_sharded(self):
        ...
        out = sc.out_src_shardings
        assert isinstance(out, spmd.SpmdType)
        self.assertEqual(_per_axis_types(out)[MeshAxisName.CP], spmd.S(0))
