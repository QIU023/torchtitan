...
def test_pipeline_with_first_last_stage_modules_refuses_layers_per_stage(
    monkeypatch,
):
    model = nn.Module()
    model.vision_encoder = nn.Linear(2, 2)
    calls = []
    monkeypatch.setattr(
        pipeline_parallel, "pipeline_llm", lambda model, **kwargs: calls.append(kwargs)
    )
    parallelism = ParallelismConfig(
        pipeline_parallel_degree=2, pipeline_parallel_layers_per_stage=3
    )

    with pytest.raises(ValueError, match="pipeline_parallel_layers_per_stage"):
        pipeline_parallel.pipeline_with_first_last_stage_modules(
            model,
    ...
    assert calls == []
    assert parallelism.pipeline_parallel_layers_per_stage == 3
    assert parallelism.pipeline_parallel_module_fqns_per_model_part is None
