...
def _worker(rank: int, world_size: int, init_file: str, out_dir: str) -> None:
    ...
    try:
        ...
        # What the CP path built before: causal only. Kept as the control, so
        # the boundary assertions below are known to discriminate rather than
        # passing against any mask.
        causal = create_mask(
            create_attention_mask(get_causal_mask_mod(), None, None, n, n, device="cpu").mask_mod,
            1,
            1,
            n,
            n,
        ...
        torch.save(
            {
                "equal": bool(torch.equal(dense, expected)),
                "dense": dense,
                "causal": causal,
                "document_only": create_mask(
                    create_attention_mask(
                        get_efficient_causal_mask_mod_for_packed_document(positions_full),
                        None,
                        None,
...
class TestUlyssesDocumentMask(unittest.TestCase):
    def test_gathered_mask_matches_global_positions(self):
        ...
        # The control admits exactly the two the document mask refuses, so a
        # regression to causal-only fails here rather than passing quietly.
        causal = results[0]["causal"].reshape(len(_POSITIONS), len(_POSITIONS))
        self.assertTrue(bool(causal[4, 3]))
        self.assertTrue(bool(causal[6, 5]))
        # The other half of the pair: the document mod on its own admits a
        # later token of the same document, so it cannot stand alone either.
        document_only = results[0]["document_only"].reshape(
            len(_POSITIONS), len(_POSITIONS)
        )
        self.assertTrue(bool(document_only[0, 1]))
        self.assertFalse(bool(dense[0, 1]))
