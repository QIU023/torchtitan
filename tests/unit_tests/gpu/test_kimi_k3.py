...
class TestKimiK3(unittest.TestCase):
    def test_preprocess_builds_token_aligned_vision_bank_indices(self):
        ...
        with (
            ...
            patch(
                "torchtitan.distributed.context_parallel.api.cp_shard_inputs",
                side_effect=lambda batch, *_args, **_kwargs: (
                    batch,
                    load_balancer,
                ),
            ) as cp_shard,
    ...
    def test_context_parallel_config_uses_kda_and_allgather_mla(self):
        ...
        for layer in model_config.layers:
            ...
            if layer.delta_attention is not None:
                ...
                self.assertEqual(
                    layer.delta_attention.inner_kda.conv_kernel_size,
    ...
    @unittest.skipIf(
        ...
        or torch.cuda.get_device_capability() not in {(8, 0), (9, 0), (10, 0), (10, 3)},
